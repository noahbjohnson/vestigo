	...
	for {
		...
		if l == 0 {
			...
			if h != nil {
				...
				if len(cn.pnames[method]) == 0 {
					cn.pnames[method] = append(cn.pnames[method], pnames[method]...)
				}
				if method == "GET" {
			...
			n := newNode(cn.typ, cn.prefix[l:], cn, cn.children, nr, pnames)
			...
			if l == sl {
				...
				if method == "GET" {
					cn.pnames["HEAD"] = cn.pnames[method]
				}
			} else {
				// Create child node
				nr := newResource()
